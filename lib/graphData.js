...
function graphArr(json, ser){
  ...
  function barGraph(arr){
    ...
    priceVolChart = makeChart(getAvgs(pZeroVals), graphHeight).concat(makeChart(getAvgs(vZeroVals), volHeight))
    priceVolChart = [stock, ['start:' + arr[arr.length-1][0], 'end:' + arr[0][0]].join(' : '), ['low:' + pLow, 'high:' + pHigh].join(' : ')].join('  ::  ') + '\n' + priceVolChart 

    return priceVolChart
  }
